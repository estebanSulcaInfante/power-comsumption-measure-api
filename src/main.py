...
# Obtener info fija del equipo
device_name = c.Win32_ComputerSystem()[0].Name
...
def main_monitor(interval=300, total_duration=3600):  # por ejemplo 1 hora total
    print("Iniciando monitor con muestreo cada 5 minutos...")
    start_time = time.time()
    data_records = []
    ...
    while time.time() - start_time < total_duration:
        ...
        # Enviar el registro inmediatamente
        post_data_chunk([record])

        time.sleep(interval)

    # Guardar todos los datos localmente al final
    save_dir = os.path.join(os.path.dirname(__file__), "..", "data")
    os.makedirs(save_dir, exist_ok=True)
    df = pd.DataFrame(data_records)
...
if __name__ == "__main__":
    ...
    main_monitor(interval=300, total_duration=3600)  # Monitoreo: muestrea cada 5 minutos por 1 hora
